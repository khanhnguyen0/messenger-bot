...
module.exports = async(senderId, message, quickReply) => {
    ...
    // If user already existed, we perform function according to user's action in apiai client
    // Otherwise, create a new user with basic information
    if (u) {
        console.log(state);
        // no state, use wit to extract the user action
        if (!state[senderId]) {
        ...
        switch (state[senderId]) {
            ...
            case 'show_menu': 
                state[senderId] = undefined;
                return send.textMessage(senderId, 'Add education - Edit education - Add experience - Edit experience - Add skill - Edit skill - Show cv')
...
const createUser = async(senderId, message) => {
    ...
    // Create a user based on info from apiai client
    const user = new User({
        id: senderId,
    ...
    user.save((err, u) => {
        ...
        console.log('USER: ', JSON.stringify(u, undefined, 2));
        if (err) {
